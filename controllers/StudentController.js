...
studentCtrl.GetAllStudents = async (req, res) => {
    try {
        ...
        if (isValidObjectId(office)) { filters.office = office } 
        if (qualification) { filters.qualification = { $regex: new RegExp(qualification, "i") } }
...
studentCtrl.GetWorkStudents = async(req,res)=>{
    ...
    try {
        ...
        const studentsFromWorks = await Work.find({assignee: new ObjectId(employeeId)}, {studentId:1});

        const studentsArray = studentsFromWorks.map((obj)=>(obj.studentId))

        const theStudents = await Student.find({_id: {$in: studentsArray}, ...filters}, {password:0})

        console.log(theStudents)

        let result = theStudents.reverse();
...
studentCtrl.GetTeamStudents = async(req,res)=>{
    ...
    try {
        ...
        const studentsFromWorks = await Work.find({assignee: {$in: memberIds}}, {studentId:1});
