...
projectCtrl.GetEmpProjects = async (req, res) => {
...
projectCtrl.AddTask = async (req, res) => {
    ...
    try {
        if (!(isValidObjectId(projectId) || isValidObjectId(assignee) || typeof taskName === 'string')) {
...
projectCtrl.DeleteATask = async (req, res) => {
    ...
    try {
        if (!isValidObjectId(taskId)) return res.status(400).json({ msg: "Invalid Task id format" })
