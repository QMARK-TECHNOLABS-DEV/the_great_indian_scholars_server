...
applicationCtrl.GetMyApplications = async (req, res) => {
        ...
        const allApplications = await Application.aggregate([
            {
                ...
                {
                    $or: [
                        { assignees: { $elemMatch: { $eq: new ObjectId(empId) } } },
                        { creator: new ObjectId(empId) }
                    ]
                }
            },
            {
...
applicationCtrl.GetTeamApplications = async (req, res) => {
...
applicationCtrl.UpdateApplication = async (req, res) => {
    const applicationId = req.params.id;
    const { country, phase } = req.body;
    console.log("updbody", req.body);
    ...
    try {
        ...
        if (phase) { updateObj.phase = phase }
